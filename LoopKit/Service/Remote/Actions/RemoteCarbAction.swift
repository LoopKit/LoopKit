...
enum RemoteCarbActionError: LocalizedError {
    ...
    public var errorDescription: String? {
             ...
             case .invalidAbsorptionTime(let absorptionTime):
                 let message = "Invalid absorption time: \(absorptionTime.hours) hours"
                 return NSLocalizedString(message, comment: "Remote command error description: invalid absorption time.")
             case .invalidStartDate(let startDate):
                 let message = "Invalid start date: \(startDate.description)"
                 return NSLocalizedString(message, comment: "Remote command error description: invalid start date.")
             }
         }
}
