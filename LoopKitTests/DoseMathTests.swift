...
struct SimpleGlucoseFixtureValue: GlucoseValue {
    let startDate: Date
...
class RecommendTempBasalTests: XCTestCase {
    ...
    func loadGlucoseValueFixture(_ resourceName: String) -> [SimpleGlucoseFixtureValue] {
        let fixture: [JSONDictionary] = loadFixture(resourceName)
        let dateFormatter = ISO8601DateFormatter.localTimeDateFormatter()

        return fixture.map {
            return SimpleGlucoseFixtureValue(
    ...
    func testNoInputGlucose() {
        let glucose: [SimpleGlucoseFixtureValue] = []
...
class RecommendBolusTests: XCTestCase {
    ...
    func testStartHighEndInRangeExponentialModel() {
        let glucose = loadGlucoseValueFixture("recommend_temp_basal_start_high_end_in_range")
    ...
    func testStartLowEndHigh() {
        ...
        XCTAssertEqual(1.7, dose.amount, accuracy: 0.05)
    ...
    func testStartLowEndHighExponentialModel() {
        let glucose = loadGlucoseValueFixture("recommend_temp_basal_start_low_end_high")
        ...
        XCTAssertEqual(1.9, dose.amount, accuracy: 0.05)
    ...
    func testStartBelowSuspendThresholdEndHighExponentialModel() {
        // 60 - 200 mg/dL
        let glucose = loadGlucoseValueFixture("recommend_temp_basal_start_low_end_high")
    ...
    func testDroppingBelowRangeThenRising() {
        ...
        XCTAssertEqual(1.575, dose.amount, accuracy: 0.05)
    ...
    func testStartLowEndJustAboveRange() {
        ...
        XCTAssertEqual(0.275, dose.amount, accuracy: 0.05)
    }
    ...
    func testHighAndRisingExponentialModel() {
        ...
        XCTAssertEqual(1.95, dose.amount, accuracy: 1.0 / 40.0)
    ...
    func testDoseWithFiaspCurve() {
        ...
        XCTAssertEqual(0.375, dose.amount, accuracy: 0.05)
    }
